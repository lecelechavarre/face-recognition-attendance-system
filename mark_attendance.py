...
class Mark_Attendance:
    ...
    def write_csv_header(self, id, date,department, student, time, status):
        self.id = id
        self.date = date
        ...
        writer = csv.DictWriter(f, fieldnames=[self.id, self.department, self.student, self.date, self.time, self.status])
        writer.writeheader()
        f.close()
